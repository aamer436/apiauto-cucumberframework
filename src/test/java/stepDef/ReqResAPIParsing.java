...
public class ReqResAPIParsing extends Utils {
@Given("Hit Get API and verify the response")
public void hit_get_api_and_verify_the_response() {
Response 
resp=given().relaxedHTTPSValidation().spec(requestSpecification(getPropValue("reqresurl"))).log().all().when().get("/api/unknown");
resp.then().log().all();
